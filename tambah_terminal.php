...
while ($row = $ambil_terminal->fetch_assoc()) {
    $terminals[] = $row['nama']; //menyimpan data nama terminal di terminals
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $koneksi->prepare("INSERT INTO terminals (nama, latitude, longitude, created_at, updated_at, cluster) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sddsss", $nama, $latitude, $longitude, $created_at, $updated_at, $cluster);

    if ($stmt->execute()) {
        echo "<script>alert('Terminal berhasil ditambahkan!'); window.location='tambah_terminal.php';</script>";
    } else {
        echo "<script>alert('Gagal menambahkan terminal.');</script>";
    }
}
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <h1 class="h2 mb-2 text-gray-800">Tambah Terminal</h1>
                    <p class="mb-4">Tambah Terminal by koordinat</p>
                    
                            <!-- Project Card Example -->
                            <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">Form Add Terminal :</h6>
                                </div>
                                ...
                                <div class="card-body">
                                    <form method="POST" action="">
                                      <div class="row mb-3">
                                        <div class="col-md-4">
                                          <label for="namaTerminal" class="form-label">Nama Terminal</label>
                                          <input type="text" class="form-control" id="namaTerminal" name="nama"  required>
                                        </div>
                                      ...
                                      <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="latitude" name="latitude" placeholder="contoh :-7.483019631297965" required>
                                      </div>
                                            
                                      <div class="mb-3">
                                        <label for="longitude" class="form-label">Longitude</label>
                                        <input type="text" class="form-control" id="longitude" name="longitude" placeholder="contoh :110.81061881012552" required>
                                      </div>
                                            
                                      <button type="submit" class="btn btn-success rounded-pill"><i class="far fa-save"></i> Simpan</button>
                                      <button type="reset" class="btn btn-danger rounded-pill"><i class="fas fa-undo"></i> Reset</button>
                                    </form>

                                </div>
                            </div>

                                                        <!-- Basic Card untuk keterangan inputan -->
                            <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-info-circle"></i> Tata Cara Penambahan Terminal</h6>
                                </div>
                                <div class="card-body">
                                    <h4><Strong>Terminal</Strong></h4> <br>
                                    Gunakan Huruf KAPITAL semua pada nama ODP/FAT/TERMINAL<br>
                                    Contoh :  B2/ODP-RTMSK/078<br>
                                    <br>
                                    <h4><Strong>Koordinat</Strong></h4>
                                    gunakan koordinat salinan google maps <br>
                                    contoh : -7.483019631297965<br>
                                    contoh : 110.81061881012552  <br>
                                    <br>
                                    <strong>>> jangan ada spasi pada koordinat ataupun tanda koma << </strong>
                                </div>
                            </div>
                </div>
                <!-- /.container-fluid -->
