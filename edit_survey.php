            ...
            <div class="card-body">
                    <form method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select name="terminal_terdekat" class="form-control" required>
                                <option value="">-- Pilih Terminal --</option>
                                <?php foreach ($terminals as $ambil_terminal): ?>
                                    <option value="<?= htmlspecialchars($ambil_terminal) ?>" <?= $pelanggan['terminal_terdekat'] == $ambil_terminal ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($ambil_terminal) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            <label>Jarak</label>
                            <input type="text" name="jarak" class="form-control" value="<?= htmlspecialchars($pelanggan['jarak']) ?>" required>
                        </div>
                        <div class="form-group">
                            <label>Status Survey</label>
                            <select name="status_survey" class="form-control" required>
                                <option value="">-- Pilih Status --</option>
                                <?php foreach ($status as $ambil_status): ?>
                                    <option value="<?= htmlspecialchars($ambil_status) ?>" <?= $pelanggan['status_survey'] == $ambil_status ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($ambil_status) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <button type="submit" class="btn btn-success rounded-pill"><i class="far fa-save"></i> Simpan Perubahan</button>
                        <a href="survey_teknis2.php" class="btn btn-warning rounded-pill">Kembali</a>
                    </form>
...
<script>
    Swal.fire({
        ...
        window.location.href = 'survey_teknis2.php';
    });
</script>
