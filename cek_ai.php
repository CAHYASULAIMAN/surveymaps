...
// Koneksi ke database
include 'koneksi.php';
$ambil_cluster = $koneksi->query("SELECT nama_cluster FROM cluster");
...
<ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar"> 
    ...
    <li class="nav-item">
        ...
        <div id="collapseSurveyTeknis" class="collapse" data-parent="#accordionSidebar">
            <div class="bg-white py-2 collapse-inner rounded">
                ...
                <a class="collapse-item" href="#">Survey AI</a>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <p class="mb-4">Check Terminal by AI <i class="fas fa-robot"></i></p>
                    
                            <!-- Project Card Example -->
                            <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">Form Check Terminal :</h6>
                                </div>
                                
                                <div class="card-body">
                                    <form id="survey-form">
                                        <input type="hidden" id="salesId" value="<?= htmlspecialchars($_SESSION['username']) ?>">
                                        <input type="hidden" name="cluster" id="clusterInput">
                                        ...
                                        <div class="mb-3">
                                          ...
                                          <input type="text" id="koordinat" class="form-control" placeholder="contoh: -7.5040, 110.8135 ->> 'usahakan selalu ada koma dan tidak typo'" required>
                                        </div>
                                        <div class="md-3">
                                                <div class="form-group md-3">
                                                  <label for="cluster" class="form-label">Cluster</label>
                                                  <select name="cluster" class="form-control" id="cluster" required>
                                                    <option value="">-- Pilih Cluster --</option>
                                                    <?php foreach ($cluster as $cl): ?>
                                        ...
                                        <a href="cek_ai.php" class="btn btn-danger rounded-pill"><i class="fas fa-undo"></i> Mulai Ulang</a>
                                    </form>
                                </div>
                                  ...
                                  <!-- Hasil datanya-->
                            <div class="card shadow mb-4" id="hasil" style="display:none">
                                <div class="card-header py-3">
                                ...
                                <div class="card-body">
                                    <!-- Informasi hasil -->
                                    <p id="hasil-nama"></p>
                                    <p id="hasil-telepon"></p>
                                    <p id="hasil-koordinat"></p>
                                    <p id="hasil-jarak"></p>
                                    <p id="hasil-terminal"></p>
                                    <div id="hasil-survey"></div> <br>
            ...
            <script>
                ...
                async function loadTerminals(cluster) {
                  const res = await fetch(`get_terminals.php?cluster=${encodeURIComponent(cluster)}`);
                ...
                if (!cluster) {
                  Swal.fire('Error', 'Harap pilih cluster terlebih dahulu', 'error');
                  return;
                }
                await loadTerminals(cluster);
            
            
                  // SweetAlert loading
                  Swal.fire({
                    title: 'Mohon tunggu...',
                    text: 'Sedang mencari terminal terdekat...',
                  ...
                  // bagian urusan koordinat agar di pisah menjadi longitude dan latitude
                  let koordinat = document.getElementById('koordinat').value.trim();           
                  // Hapus simbol derajat dan spasi berlebih
                  koordinat = koordinat.replace(/°/g, '').replace(/\s+/g, '');
                  // Split dan parsing angka
                  const [lat, lng] = koordinat.split(',').map(coord => parseFloat(coord));
                  if (isNaN(lat) || isNaN(lng)) {
                    Swal.close();
                    Swal.fire({
                      icon: 'error',
                      title: 'Format koordinat tidak valid',
                      text: 'Pastikan koordinat dalam format seperti: -7.505242,110.812601',
                  ...
                  for (const terminal of terminals) {
                    const url = `https://router.project-osrm.org/route/v1/driving/${lng},${lat};${terminal.lng},${terminal.lat}?overview=full&geometries=geojson`;
                    ...
                    try {
                      ...
                      if (data.routes && data.routes.length > 0) {
                        ...
                        if (distance < minDistance) {
                          minDistance = 25 + distance;
                          nearestTerminal = terminal;
                          bestRoute = data.routes[0].geometry;
                        }
                      }
                    } catch (err) {
                  ...
                  const status_survey = minDistance > 100 ? 'Survey Teknis' : 'Bisa';

              
                  // Tampilkan hasil ke halaman
                  document.getElementById('hasil-nama').textContent = `Nama : ${nama}`;
                  document.getElementById('hasil-telepon').textContent = `Telepon : ${telepon}`;
                  document.getElementById('hasil-koordinat').textContent = `Koordinat : ${lat}, ${lng}`;
                  document.getElementById('hasil-jarak').textContent = `Jarak ke terminal terdekat : ${minDistance.toFixed(2)} meter`;
                  document.getElementById('hasil-terminal').textContent = `Nama ODP (Terminal) : ${nearestTerminal.nama}`;
                //   document.getElementById('hasil-survey').textContent = `${status_survey}`;

                  const hasilSurvey = document.getElementById('hasil-survey');

                    if (status_survey === 'Survey Teknis') {
                      hasilSurvey.innerHTML = `
                        <a href="#" class="btn btn-warning btn-icon-split">
                          <span class="icon text-white-50">
                            <i class="fas fa-exclamation-triangle"></i>
                          </span>
                          <span class="text">Survey Teknis</span>
                        </a>`;
                    } else if (status_survey === 'Bisa') {
                      hasilSurvey.innerHTML = `
                        <a href="#" class="btn btn-success btn-icon-split">
                          <span class="icon text-white-50">
                            <i class="fas fa-check"></i>
                          </span>
                          <span class="text">Bisa</span>
                        </a>`;
                    }
                  ...
                  // Simpan ke database
                  await fetch('simpan_pelanggan.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: new URLSearchParams({
                      nama, sales: salesId, telepon, alamat, lat, lng,
                      terminal: nearestTerminal.nama,
                      status_survey, keterangan, tanggal_po,
                      jarak: minDistance
                    })
                  });
                  ...
                  const text = `📡 *HASIL SURVEY BARU!*\n\n` +
                    `==============================\n` +
                    `🔧 *Status Survey:* *${status_survey}\n*` +
                    `📏 *Jarak        :* ${minDistance.toFixed(2)} meter\n` +
                    `📶 *Terminal Terdekat:*\n    ${nearestTerminal ? nearestTerminal.nama : '-'}\n\n` +
                    `==============================\n` +
                    `👤 *Nama    :* ${nama}\n` +
                    `📞 *Telepon :* ${telepon}\n` +
                    `📍 *Koordinat:*\n    ${lat},    ${lng}\n` +
                  ...
                  // Tutup SweetAlert & tampilkan sukses
                  Swal.close();
                  Swal.fire({
                    icon: 'success',
                    title: 'Selesai!',
                    text: 'Terminal terdekat berhasil ditemukan dan data telah disimpan!',
                    confirmButtonText: 'Oke'
                  });
                  ...
                  L.marker([lat, lng]).addTo(map).bindPopup("Pelanggan").openPopup();
                  L.marker([nearestTerminal.lat, nearestTerminal.lng]).addTo(map).bindPopup(nearestTerminal.nama);
                  if (bestRoute) {
                    L.geoJSON(bestRoute, { color: 'blue' }).addTo(map);
                
                // Jika map sudah ada, paksa update ukurannya
                    setTimeout(() => {
                      map.invalidateSize();
                    }, 300); // Delay kecil agar benar-benar selesai render
  ...
  if (!document.fullscreenElement &&
    ...
    <script>
        function selectCluster(value) {
            document.getElementById("clusterInput").value = value;
